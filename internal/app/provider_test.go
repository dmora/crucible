...
func setupMockProviders() map[string]config.ProviderConfig {
	return map[string]config.ProviderConfig{
		"openai": {
			ID:     "openai",
			Name:   "OpenAI",
			Models: []config.ModelMetadata{{ID: "gpt-4o"}, {ID: "gpt-4o-mini"}},
		},
		"anthropic": {
			ID:     "anthropic",
			Name:   "Anthropic",
			Models: []config.ModelMetadata{{ID: "claude-3-sonnet"}, {ID: "claude-3-opus"}},
		},
	}
}
...
func setupMockProvidersWithSlashes() map[string]config.ProviderConfig {
	return map[string]config.ProviderConfig{
		...
		"openai": {
			ID:     "openai",
			Name:   "OpenAI",
			Models: []config.ModelMetadata{{ID: "gpt-4o"}},
		},
	}
}
		...
		},
		{
			...
			setupProviders: func() map[string]config.ProviderConfig {
				return map[string]config.ProviderConfig{
					"openai": {
						ID:     "openai",
						Models: []config.ModelMetadata{{ID: "shared-model"}},
					},
					"anthropic": {
						ID:     "anthropic",
						Models: []config.ModelMetadata{{ID: "shared-model"}},
