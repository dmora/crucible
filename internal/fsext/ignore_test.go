...
func TestCrucibleIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .crucibleignore file that ignores .log files
	require.NoError(t, os.WriteFile(".crucibleignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .crucibleignore file
	crucibleignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".crucibleignore"), []byte(crucibleignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .crucibleignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .crucibleignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .crucibleignore in subdir that ignores normal_nested
	subCrucibleignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".crucibleignore"), []byte(subCrucibleignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .crucibleignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .crucibleignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .crucibleignore")
