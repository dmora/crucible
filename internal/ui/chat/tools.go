...
import (
	...
	"github.com/dmora/crucible/internal/stringext"
...
// IsPending returns true if the tool call is still pending (not terminal and
// not canceled).
func (o *ToolRenderOpts) IsPending() bool {
	return !o.ToolCall.State.IsTerminal() && !o.IsCanceled()
}
...
// Tool spinners are disabled — station cards have activity trees, todos have
// the pill spinner, and other tools show status text. The wide meter added
...
func pendingTool(sty *styles.Styles, name string, a anim.SpinnerBackend) string {
...
func toolEarlyStateContent(sty *styles.Styles, opts *ToolRenderOpts, width int) (string, bool) {
	...
	case ToolStatusAwaitingInput:
		msg = sty.Tool.StateWaiting.Render("Waiting for operator response...")
...
// roundedIndenter creates a tree indenter matching the visual width of
// roundedEnumerator. For non-last children it produces "│" (continuation),
// for the last child it produces spaces. Total width: lPadding + 1 + width.
...
func (t *baseToolMessageItem) formatParametersForCopy() string {
	...
	if json.Unmarshal([]byte(t.toolCall.Input), &params) == nil {
		...
		for key, value := range params {
			...
			parts = append(parts, fmt.Sprintf("**%s:** %s", displayKey, displayValue))
		}
		return strings.Join(parts, "\n")
	}
...
func (t *baseToolMessageItem) formatResultForCopy() string {
	...
	case tools.DownloadToolName, tools.GrepToolName, tools.GlobToolName, tools.LSToolName, tools.SourcegraphToolName, tools.TodosToolName:
		return fmt.Sprintf("```\n%s\n```", t.result.Content)
	default:
