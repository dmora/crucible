...
// agentToolName is the tool name for the sub-agent tool call.
const agentToolName = "agent"

// agentParams represents the parameters for an agent tool call.
...
func (r *AgentToolRenderContext) RenderTool(sty *styles.Styles, width int, opts *ToolRenderOpts) string {
	...
	if !opts.ToolCall.State.IsTerminal() && !opts.IsCanceled() && len(r.agent.nestedTools) == 0 {
	...
	// Tree prefix width: enumerator string + 1-char gap the tree renderer
	// adds between the enumerator/indenter and the child content.
	treeLPadding := 2
	treeDashWidth := max(taskTagWidth-5, 2)
	treePrefixWidth := treeLPadding + 1 + treeDashWidth + 1
	childWidth := cappedWidth - treePrefixWidth

	// Build tree with nested tool calls.
	childTools := tree.Root(header)

	for _, nestedTool := range r.agent.nestedTools {
		// nestedTool.Render() → RawRender() subtracts MessageLeftPaddingTotal internally.
		// Compensate so the nested tool content fits within childWidth.
		childView := nestedTool.Render(childWidth + MessageLeftPaddingTotal)
...
func (r *AgenticFetchToolRenderContext) RenderTool(sty *styles.Styles, width int, opts *ToolRenderOpts) string {
	cappedWidth := cappedMessageWidth(width)
	if !opts.ToolCall.State.IsTerminal() && !opts.IsCanceled() && len(r.fetch.nestedTools) == 0 {
	...
	// adds between the enumerator/indenter and the child content.
	treeLPadding := 2
	treeDashWidth := max(promptTagWidth-5, 2)
	treePrefixWidth := treeLPadding + 1 + treeDashWidth + 1
	childWidth := cappedWidth - treePrefixWidth
