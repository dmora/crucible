...
// AssistantInfoItem renders token usage and response time after assistant completes.
type AssistantInfoItem struct {
	*cachedMessageItem
	...
	prevPromptTokens    int64 // prompt tokens from the previous turn (for per-turn delta)
}

// SetMessage updates the underlying message and clears the render cache.
...
func NewAssistantInfoItem(sty *styles.Styles, message *message.Message, lastUserMessageTime time.Time, prevPromptTokens int64) MessageItem {
...
func (a *AssistantInfoItem) renderContent(width int) string {
	...
	durationStr := a.sty.Chat.Message.AssistantInfoDuration.Render(duration.String())

	var parts []string
	// Show per-turn input tokens (delta from previous turn's cumulative prompt tokens).
	inputTokens := finishData.PromptTokens - a.prevPromptTokens
	if inputTokens < 0 {
		inputTokens = finishData.PromptTokens // first turn or data unavailable
	}
	if inputTokens > 0 || finishData.CandidatesTokens > 0 {
		input := common.FormatTokenCount(inputTokens)
		output := common.FormatTokenCount(finishData.CandidatesTokens)
		tokenInfo := a.sty.Chat.Message.AssistantInfoModel.Render(
			fmt.Sprintf("↑ %s · ↓ %s tokens", input, output),
		)
		parts = append(parts, tokenInfo)
	}
	parts = append(parts, durationStr)

	assistant := fmt.Sprintf("%s %s",
		a.sty.Chat.Message.AssistantInfoIcon.Render(styles.ModelIcon),
		strings.Join(parts, " · "),
	)
...
func ExtractMessageItems(sty *styles.Styles, msg *message.Message, toolResults map[string]message.ToolResult) []MessageItem {
	...
	case message.Assistant:
		...
		// Show grounding (Google Search) before the response text.
		if gc := msg.Grounding(); gc != nil {
			items = append(items, NewGroundingMessageItem(sty, msg.ID, gc))
		}
