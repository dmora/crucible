...
const (
	...
	// rebirthInterval is how often the birth animation replays, creating
	// a breathing/pulse effect for long-running spinners.
	rebirthInterval = 5 * time.Second
...
var (
	availableRunes = []rune("0123456789ABCDEF░▒▓█")
...
func settingsHash(opts Settings) string {
	...
	fmt.Fprintf(h, "%d-%s-%v-%v-%v-%v-%t",
		opts.Size, opts.Label, opts.LabelColor, opts.GradColorA, opts.GradColorB, opts.GradColorC, opts.CycleColors)
...
type Settings struct {
	...
	GradColorC  color.Color // optional third gradient stop for richer color cycling
	CycleColors bool
}
...
type Anim struct {
	...
	id               string
	glitchFrame      int // counter for glitch truncation pacing
	glitchWidth      int // current randomized truncation width (held across frames)
}
...
func New(opts Settings) *Anim {
	...
	if exists {
		...
		has3Colors := !colorIsUnset(opts.GradColorC)
		if opts.CycleColors {
			if has3Colors {
				// Ping-pong: A→B→C→B→A so wrapping creates a seamless infinite loop.
				ramp = makeGradientRamp(a.width*5, opts.GradColorA, opts.GradColorB, opts.GradColorC, opts.GradColorB, opts.GradColorA, opts.GradColorB)
				numFrames = a.width * 4
			} else {
				// Ping-pong: A→B→A so wrapping creates a seamless infinite loop.
				ramp = makeGradientRamp(a.width*3, opts.GradColorA, opts.GradColorB, opts.GradColorA, opts.GradColorB)
				numFrames = a.width * 2
			}
		} else {
			if has3Colors {
				ramp = makeGradientRamp(a.width, opts.GradColorA, opts.GradColorB, opts.GradColorC)
			} else {
...
func (a *Anim) Render() string {
	...
	for i := range a.width {
		switch {
		case !a.initialized.Load() && i < len(a.birthOffsets) && time.Since(a.cycleStart) < a.birthOffsets[i]:
	...
	// Glitch effect: randomly truncate the visible width every few frames
	// for a slow pulse rather than a frantic flicker. Hold duration scales
	// with spinner width so small spinners get more variation.
	w := lipgloss.Width(rendered)
	if w > 4 {
		// Scale hold: wider spinners hold longer (more visual area to absorb),
		// narrow spinners cycle faster (fewer distinct widths available).
		holdFrames := max(2, min(w/4, 6)) //nolint:gosec
		if a.glitchWidth == 0 {
			// First render: show full width before glitching starts.
			a.glitchWidth = w
		}
		a.glitchFrame++
		if a.glitchFrame >= holdFrames {
			// Bias toward wider values (70-100% of width) with occasional
			// deep truncation for dramatic flicker.
			if rand.IntN(4) == 0 { //nolint:gosec
				// 25% chance: deep truncation (30-60% of width)
				a.glitchWidth = rand.IntN(w*3/10) + w*3/10 //nolint:gosec
			} else {
				// 75% chance: subtle truncation (70-100% of width)
				a.glitchWidth = rand.IntN(w*3/10) + w*7/10 //nolint:gosec
			}
			a.glitchFrame = 0
		}
		rendered = ansi.Truncate(rendered, min(a.glitchWidth, w), "")
	}

	return rendered
}

// FollowsText returns true — the industrial spinner chases streamed text.
func (a *Anim) FollowsText() bool { return true }
...
// Used for event-driven spinners (advance on activity, not on timer).
func (a *Anim) StepOnce() {
	a.advanceFrame()
}

// advanceFrame advances the frame counter and manages initialization/rebirth/ellipsis state.
func (a *Anim) advanceFrame() {
	...
	if a.initialized.Load() {
		...
		if time.Since(a.cycleStart) >= rebirthInterval {
	...
	} else if time.Since(a.cycleStart) >= maxBirthOffset {
