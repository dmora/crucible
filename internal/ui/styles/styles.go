...
import (
	...
	"github.com/dmora/crucible/internal/ui/diffview"
)
...
type Styles struct {
	...
	Header struct {
		...
		Designation  lipgloss.Style // Style for status designation (e.g., "SYS:ONLINE")
	}
	...
	EditorPromptHoldIconFocused  lipgloss.Style
	EditorPromptHoldIconBlurred  lipgloss.Style
	EditorPromptHoldDotsFocused  lipgloss.Style
	EditorPromptHoldDotsBlurred  lipgloss.Style
	EditorPromptShellIconFocused lipgloss.Style
	...
	// SpinnerPreset is the active spinner style name (not a lipgloss style).
	// Set by the caller after construction, not by buildStyles.
	...
	Tool struct {
		...
		// Station chip styles (visually prominent for intervention states)
		StationChipError   lipgloss.Style // Failed state chip (redDark foreground)
		StationChipWarning lipgloss.Style // Permission-waiting state chip (warning foreground)
	...
	Status struct {
		...
		Metrics        lipgloss.Style // context metrics (percentage)
		MetricsMuted   lipgloss.Style // muted elements (cost, token count)
		MetricsWarning lipgloss.Style // warning style (>80% context)
	}
...
// NewStyles returns styles for the given theme. Falls back to DefaultTheme on unknown IDs.
// Palettes are run through DeriveDefaults (to fill zero-valued Tier 2–4 fields)
// and ValidatePalette (to catch broken palettes) before building styles.
func NewStyles(id ThemeID, transparent bool) Styles {
	p, err := LookupPalette(id)
	if err != nil {
		p, _ = LookupPalette(DefaultTheme)
	}
	p = DeriveDefaults(p)
	if err := ValidatePalette(p); err != nil {
		// Broken palette — fall back to a known-good default.
		p, _ = LookupPalette(DefaultTheme)
		p = DeriveDefaults(p)
	}
	return buildStyles(p, transparent)
}

// DefaultStyles returns the default (steel-blue) styles for the UI.
// Preserved for backward compatibility.
func DefaultStyles() Styles {
	return NewStyles(DefaultTheme, false)
}

// buildStyles constructs a full Styles from a Palette. This is the single
// source of truth for mapping palette colors to UI styles.
func buildStyles(p Palette, transparent bool) Styles { //nolint:cyclop,maintidx,funlen
	var (
		primary       = p.Primary
	...
	// In transparent mode, content/panel backgrounds become "no background"
	// so the terminal shows through. Accent backgrounds (tags, headers, buttons) are kept.
	// panelBg conditionally applies a background — no-op in transparent mode.
	panelBg := func(s lipgloss.Style, c color.Color) lipgloss.Style {
		if transparent {
			return s
		}
		return s.Background(c)
	}
	// panelBgColor returns the color in opaque mode, nil in transparent mode.
	panelBgColor := func(c color.Color) color.Color {
		if transparent {
			return nil
		}
		return c
	...
	// Glamour BackgroundColor fields are *string — nil means no background.
	var codeBgStr *string
	var plainBgStr *string
	if !transparent {
		codeBgStr = stringPtr(hexStr(bgSubtle))
		plainBgStr = stringPtr(hexStr(bgBaseLighter))
	}
	...
	s.Markdown = ansi.StyleConfig{
		Document: ansi.StyleBlock{
			StylePrimitive: ansi.StylePrimitive{
				// BlockPrefix: "\n",
				// BlockSuffix: "\n",
				Color: stringPtr(hexStr(fgHalfMuted)),
			},
		...
		H1: ansi.StyleBlock{
			StylePrimitive: ansi.StylePrimitive{
				Prefix:          " ",
				Suffix:          " ",
				Color:           stringPtr(hexStr(warning)),
				BackgroundColor: stringPtr(hexStr(primary)),
				Bold:            boolPtr(true),
			},
		},
		H2: ansi.StyleBlock{
		...
		H6: ansi.StyleBlock{
			StylePrimitive: ansi.StylePrimitive{
				Prefix: "###### ",
				Color:  stringPtr(hexStr(greenDark)),
				Bold:   boolPtr(false),
			},
		},
		...
		HorizontalRule: ansi.StylePrimitive{
			Color:  stringPtr(hexStr(bgSubtle)),
			Format: "\n--------\n",
		},
		...
		Link: ansi.StylePrimitive{
			Color:     stringPtr(hexStr(medSteel)),
		...
		Image: ansi.StylePrimitive{
			Color:     stringPtr(hexStr(secondary)),
			Underline: boolPtr(true),
		},
		ImageText: ansi.StylePrimitive{
			Color:  stringPtr(hexStr(fgMuted)),
			Format: "Image: {{.text}} →",
		},
		Code: ansi.StyleBlock{
			StylePrimitive: ansi.StylePrimitive{
				Prefix:          " ",
				Suffix:          " ",
				Color:           stringPtr(hexStr(red)),
				BackgroundColor: codeBgStr,
			},
		},
		CodeBlock: ansi.StyleCodeBlock{
			...
			Chroma: &ansi.Chroma{
				...
				Error: ansi.StylePrimitive{
					Color:           stringPtr(hexStr(white)),
					BackgroundColor: stringPtr(hexStr(errColor)),
				},
				Comment: ansi.StylePrimitive{
					Color: stringPtr(hexStr(fgSubtle)),
				},
				...
				KeywordType: ansi.StylePrimitive{
					Color: stringPtr(hexStr(primary)),
				},
				Operator: ansi.StylePrimitive{
					Color: stringPtr(hexStr(operator)),
				},
				...
				NameTag: ansi.StylePrimitive{
					Color: stringPtr(hexStr(blueLight)),
				},
				NameAttribute: ansi.StylePrimitive{
					Color: stringPtr(hexStr(medSteel)),
				},
				NameClass: ansi.StylePrimitive{
					Color:     stringPtr(hexStr(salt)),
					Underline: boolPtr(true),
					Bold:      boolPtr(true),
				},
				...
				LiteralNumber: ansi.StylePrimitive{
					Color: stringPtr(hexStr(green)),
				},
				LiteralString: ansi.StylePrimitive{
					Color: stringPtr(hexStr(warmGray)),
				},
				LiteralStringEscape: ansi.StylePrimitive{
					Color: stringPtr(hexStr(greenLight)),
				},
	...
	plainBg := plainBgStr
	plainFg := stringPtr(hexStr(fgMuted))
	...
	s.Diff = diffview.Style{
		DividerLine: diffview.LineStyle{
			LineNumber: panelBg(lipgloss.NewStyle().
				Foreground(fgHalfMuted), bgBaseLighter),
			Code: panelBg(lipgloss.NewStyle().
				Foreground(fgHalfMuted), bgBaseLighter),
		},
		MissingLine: diffview.LineStyle{
			LineNumber: panelBg(lipgloss.NewStyle(), bgBaseLighter),
			Code:       panelBg(lipgloss.NewStyle(), bgBaseLighter),
		},
		EqualLine: diffview.LineStyle{
			LineNumber: panelBg(lipgloss.NewStyle().
				Foreground(fgMuted), bgBase),
			Code: panelBg(lipgloss.NewStyle().
				Foreground(fgMuted), bgBase),
		},
	...
	// Compact header styles — solid strip with colored background
	headerBg := primary
	s.Header.Charm = base.Foreground(secondary).Background(headerBg)
	s.Header.Diagonals = base.Foreground(bgBase).Background(headerBg)
	s.Header.Percentage = base.Foreground(bgBase).Background(headerBg)
	s.Header.Keystroke = base.Foreground(bgBase).Background(headerBg)
	s.Header.KeystrokeTip = base.Foreground(bgSubtle).Background(headerBg)
	s.Header.WorkingDir = base.Foreground(bgBase).Background(headerBg)
	s.Header.Separator = base.Foreground(bgSubtle).Background(headerBg)
	s.Header.Designation = base.Foreground(bgBase).Background(headerBg).Bold(true)
	...
	// panels
	s.PanelMuted = panelBg(s.Muted, bgBaseLighter)
	s.PanelBase = panelBg(lipgloss.NewStyle(), bgBase)

	// code line number
	s.LineNumber = panelBg(lipgloss.NewStyle().Foreground(fgMuted), bgBase).PaddingRight(1).PaddingLeft(1)
	...
	s.Tool.ContentLine = panelBg(s.Muted, bgBaseLighter)
	s.Tool.ContentTruncation = panelBg(s.Muted, bgBaseLighter)
	s.Tool.ContentCodeLine = panelBg(s.Base, bgBase).PaddingLeft(2)
	s.Tool.ContentCodeTruncation = panelBg(s.Muted, bgBase).PaddingLeft(2)
	s.Tool.ContentCodeBg = panelBgColor(bgBase)
	s.Tool.Body = base.PaddingLeft(2)

	// Deprecated - kept for backward compatibility
	s.Tool.ContentBg = panelBg(s.Muted, bgBaseLighter)
	s.Tool.ContentText = s.Muted
	s.Tool.ContentLineNumber = panelBg(base.Foreground(fgMuted), bgBase).PaddingRight(1).PaddingLeft(1)
	...
	s.Tool.StationChipError = base.Foreground(redDark)
	s.Tool.StationChipWarning = base.Foreground(warning)

	// Diff and multi-edit styles
	s.Tool.DiffTruncation = panelBg(s.Muted, bgBaseLighter).PaddingLeft(2)
	...
	s.EditorPromptYoloIconFocused = lipgloss.NewStyle().MarginRight(1).Foreground(fgSubtle).Background(warning).Bold(true).SetString(" ! ")
	...
	s.EditorPromptHoldIconFocused = lipgloss.NewStyle().MarginRight(1).Foreground(fgSubtle).Background(errColor).Bold(true).SetString(" ‖ ")
	s.EditorPromptHoldIconBlurred = s.EditorPromptHoldIconFocused.Foreground(bgBase).Background(fgMuted)
	s.EditorPromptHoldDotsFocused = lipgloss.NewStyle().MarginRight(1).Foreground(errColor).SetString(":::")
	s.EditorPromptHoldDotsBlurred = s.EditorPromptHoldDotsFocused.Foreground(fgMuted)
	s.EditorPromptShellIconFocused = lipgloss.NewStyle().MarginRight(1).Foreground(fgSubtle).Background(info).Bold(true).SetString(" $ ")
	...
	// Thinking section styles
	s.Chat.Message.ThinkingBox = panelBg(s.Subtle, bgBaseLighter)
	...
	// Text selection.
	s.TextSelection = lipgloss.NewStyle().Foreground(salt).Background(primary)
	...
	s.Status.Metrics = base.Foreground(fgBase)
	s.Status.MetricsMuted = base.Foreground(fgMuted)
	s.Status.MetricsWarning = base.Foreground(yellow)

	// Completions styles
	s.Completions.Normal = panelBg(base, bgSubtle).Foreground(fgBase)
	...
	s.Pills.Area = base
	s.Pills.TodoSpinner = base.Foreground(primary)
...
func chromaStyle(style ansi.StylePrimitive) string { //nolint:gocognit
