...
// ApplyBoldForegroundGradWithBg renders a given string with a horizontal
// gradient foreground and a solid background color.
func ApplyBoldForegroundGradWithBg(t *Styles, input string, color1, color2, bg color.Color) string {
	if input == "" {
		return ""
	}
	var o strings.Builder
	clusters := ForegroundGradWithBg(t, input, true, color1, color2, bg)
	for _, c := range clusters {
		fmt.Fprint(&o, c)
	}
	return o.String()
...
// ForegroundGradWithBg is like ForegroundGrad but also sets a background color.
func ForegroundGradWithBg(t *Styles, input string, bold bool, color1, color2, bg color.Color) []string {
	if input == "" {
		return []string{""}
	}
	if len(input) == 1 {
		style := t.Base.Foreground(color1).Background(bg)
		if bold {
			style.Bold(true)
		}
		return []string{style.Render(input)}
	...
	ramp := blendColors(len(clusters), color1, color2)
	for i, c := range ramp {
		style := t.Base.Foreground(c).Background(bg)
		if bold {
			style.Bold(true)
		}
		clusters[i] = style.Render(clusters[i])
