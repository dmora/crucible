...
import (
	...
	"github.com/dmora/crucible/internal/ui/attachments"
	"github.com/dmora/crucible/internal/ui/chat"
	"github.com/dmora/crucible/internal/ui/common"
	"github.com/dmora/crucible/internal/ui/completions"
	"github.com/dmora/crucible/internal/ui/dialog"
	fimage "github.com/dmora/crucible/internal/ui/image"
...
// reinitReadyMsg is sent after the init flag file has been deleted,
// signaling that the UI should transition to the uiInitialize state.
type reinitReadyMsg struct{ err error }
...
type UI struct {
	...
	prevPromptTokens    int64 // prompt tokens from previous turn (for per-turn delta)
	...
	pendingSessionID string // tracks the most recent session load request
	...
	// Turn metrics tick is running
	turnMetricsTicking bool
...
func New(com *common.Common) *UI {
	...
	ui := &UI{
		...
		processStates: make(map[string]agent.ProcessInfo),
	}

	status := NewStatus(com, ui)

	ui.textarea.SetPromptFunc(4, ui.editorPromptFunc)
	...
	// Register station names so the chat renders them with the station widget.
	if stations := com.Config().Stations; len(stations) > 0 {
		names := make([]string, 0, len(stations))
		for name := range stations {
			names = append(names, name)
		}
		chat.RegisterStationNames(names)
	}
...
func (m *UI) Init() tea.Cmd {
	...
	// start startup glitch animation
...
func (m *UI) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case glitchTickMsg:
		if m.header.glitchActive || m.isAgentBusy() {
			cmds = append(cmds, glitchTick())
		}
	case turnMetricsTickMsg:
		m.invalidateActiveStationCaches()
		if m.isAgentBusy() {
			cmds = append(cmds, turnMetricsTick())
		} else {
			m.turnMetricsTicking = false
		}
	case loadSessionMsg:
		// Discard stale loads from a previous session switch.
		...
		m.processStates = msg.processStates
		m.dispatchLog = msg.dispatchLog
	...
	case pubsub.Event[message.Message]:
		...
		if hasInProgressTodo(m.session.Todos) && m.isAgentBusy() && !m.todoIsSpinning {
			m.todoIsSpinning = true
			cmds = append(cmds, m.todoSpinner.Tick, glitchTick())
		}
		// stop the spinner if the agent is not busy anymore
		if m.todoIsSpinning && !m.isAgentBusy() {
			m.todoIsSpinning = false
		}
		// start/stop turn metrics tick
		if m.isAgentBusy() && !m.turnMetricsTicking {
			m.turnMetricsTicking = true
			cmds = append(cmds, turnMetricsTick())
		}
		if !m.isAgentBusy() && m.turnMetricsTicking {
			m.turnMetricsTicking = false
		}
		m.stepActiveStationSpinners()
		// there is a number of things that could change the pills here so we want to re-render
		m.renderPills()
	case pubsub.Event[history.File]:
		cmds = append(cmds, m.handleFileEvent(msg.Payload))
	case pubsub.Event[agent.ProcessEvent]:
		m.processStates = agent.GetProcessStates()
		m.stepActiveStationSpinners()
	...
	case pubsub.Event[askuser.Request]:
		if cmd := m.openAskUserDialog(msg.Payload); cmd != nil {
			cmds = append(cmds, cmd)
		}
	...
	case reinitReadyMsg:
		if msg.err != nil {
			cmds = append(cmds, util.ReportWarn("Failed to delete init flag: "+msg.err.Error()))
			break
		}
	...
	case anim.StepMsg:
		if m.state == uiChat {
			...
			// Keep viewport synced with typing reveal during streaming.
			if m.chat.Follow() && m.isAgentBusy() {
	...
	// Placeholder: frame-level single source of truth.
	// Turn metrics override everything when busy (except mode indicators).
	if m.isAgentBusy() {
...
func (m *UI) setSessionMessages(msgs []message.Message) tea.Cmd {
	...
	for _, msg := range msgPtrs {
		...
		case message.Assistant:
			...
			if fp := msg.FinishPart(); fp != nil && fp.Reason == message.FinishReasonEndTurn {
				infoItem := chat.NewAssistantInfoItem(m.com.Styles, msg, time.Unix(m.lastUserMessageTime, 0), m.prevPromptTokens)
				items = append(items, infoItem)
				m.prevPromptTokens = fp.PromptTokens
			}
		default:
			items = append(items, chat.ExtractMessageItems(m.com.Styles, msg, toolResultMap)...)
		}
	...
	// Prepend factory status banner for new (empty) sessions.
	if len(msgs) == 0 {
		bannerCfg := chat.BuildBannerConfig(m.com.Config(), m.mcpStates)
		banner := chat.NewBannerMessageItem(m.com.Styles, bannerCfg)
		items = append([]chat.MessageItem{banner}, items...)
	}
	...
	// to make sure the animations are shown. Also set the spinner label for
	// classic (non-FollowsText) spinners so they show the session title.
...
func (m *UI) loadNestedToolCalls(items []chat.MessageItem) {
	for _, item := range items {
		...
		agentSessionID := session.CreateAgentToolSessionID(messageID, tc.ID)
...
func (m *UI) updateSessionMessage(msg message.Message) tea.Cmd {
	...
	// Propagate cancellation to all in-flight tool items so spinners stop.
	if fp := msg.FinishPart(); fp != nil && fp.Reason == message.FinishReasonCanceled {
		for _, tc := range msg.ToolCalls() {
			if existing := m.chat.MessageItem(tc.ID); existing != nil {
				if toolItem, ok := existing.(chat.ToolMessageItem); ok {
	...
	if fp := msg.FinishPart(); fp != nil && shouldRenderAssistant && fp.Reason == message.FinishReasonEndTurn {
		if existing := m.chat.MessageItem(chat.AssistantInfoID(msg.ID)); existing != nil {
			if infoItem, ok := existing.(*chat.AssistantInfoItem); ok {
				infoItem.SetMessage(&msg)
			}
		} else {
			newInfoItem := chat.NewAssistantInfoItem(m.com.Styles, &msg, time.Unix(m.lastUserMessageTime, 0), m.prevPromptTokens)
	...
	// Add grounding widget if grounding metadata arrived.
	if gc := msg.Grounding(); gc != nil {
		groundingID := msg.ID + ":grounding"
		if m.chat.MessageItem(groundingID) == nil {
			items = append(items, chat.NewGroundingMessageItem(m.com.Styles, msg.ID, gc))
		}
	}
	...
	for _, tc := range msg.ToolCalls() {
		...
		if toolItem, ok := existingToolItem.(chat.ToolMessageItem); ok {
			...
			// only update if state or input changed to avoid clearing the cache
			if tc.State != existingToolCall.State || tc.Input != existingToolCall.Input {
...
func (m *UI) handleDialogMsg(msg tea.Msg) tea.Cmd {
	...
	case dialog.ActionSelectSession:
		...
		m.pendingSessionID = msg.Session.ID
	...
	case dialog.ActionToggleThinking:
		cmds = append(cmds, func() tea.Msg {
			...
			agentCfg, ok := cfg.Agents[config.AgentCrucible]
			if !ok {
	...
	case dialog.ActionInitializeProject:
		...
		// Delete init flag asynchronously, then transition to uiInitialize
		cfg := m.com.Config()
		cmds = append(cmds, func() tea.Msg {
			flagPath := filepath.Join(cfg.Options.DataDirectory, config.InitFlagFilename)
			err := os.Remove(flagPath) // may not exist — that's fine
			if err != nil && !os.IsNotExist(err) {
				return reinitReadyMsg{err: err}
			}
			return reinitReadyMsg{}
		})
		m.dialog.CloseDialog(dialog.CommandsID)
	...
	case dialog.ActionSelectTheme:
		m.dialog.CloseDialog(dialog.ThemeDialogID)
		if cmd := m.applyTheme(msg.ThemeID); cmd != nil {
			cmds = append(cmds, cmd)
		}
	case dialog.ActionSetTransparent:
		m.isTransparent = msg.Transparent
		if err := m.com.Config().SetTransparent(msg.Transparent); err != nil {
			cmds = append(cmds, util.ReportError(err))
			break
		}
		// Rebuild styles with new transparent flag
		themeID := m.com.Config().ThemeID()
	...
	case dialog.ActionShowFactoryStatus:
		m.dialog.CloseFrontDialog()
		if m.session != nil {
			bannerCfg := chat.BuildBannerConfig(m.com.Config(), m.mcpStates)
			banner := chat.NewBannerMessageItem(m.com.Styles, bannerCfg)
			m.chat.AppendMessages(banner)
			m.chat.ScrollToBottom()
		}
	case dialog.ActionShowEquipment:
		m.dialog.CloseFrontDialog()
		if cmd := m.openEquipmentDialog(); cmd != nil {
			cmds = append(cmds, cmd)
		}
	default:
...
func (m *UI) openAuthenticationDialog(provider config.ProviderMetadata, model config.SelectedModel, modelType config.SelectedModelType) tea.Cmd {
	isOnboarding := m.state == uiOnboarding
...
func (m *UI) handleKeyPressMsg(msg tea.KeyPressMsg) tea.Cmd {
	...
	handleGlobalKeys := func(msg tea.KeyPressMsg) bool {
		...
		case key.Matches(msg, m.keyMap.Theme):
			...
			return true
		case key.Matches(msg, m.keyMap.Artifacts):
			if cmd := m.openArtifactsDialog(); cmd != nil {
				cmds = append(cmds, cmd)
			}
			return true
		case key.Matches(msg, m.keyMap.Equipment):
			if cmd := m.openEquipmentDialog(); cmd != nil {
				cmds = append(cmds, cmd)
			}
			return true
		...
		case key.Matches(msg, m.keyMap.Suspend):
			...
			return true
		case key.Matches(msg, m.keyMap.Hold):
			if cmd := m.toggleHold(); cmd != nil {
				cmds = append(cmds, cmd)
				return true
			}
		...
		case uiFocusEditor:
			...
			case key.Matches(msg, m.keyMap.Editor.SendMessage):
				...
				if strings.HasPrefix(value, "!") {
					cmd := strings.TrimSpace(strings.TrimPrefix(value, "!"))
					if cmd == "" {
						return nil
					}
					return m.executeShellCommand(cmd)
				}
...
func (m *UI) Draw(scr uv.Screen, area uv.Rectangle) *tea.Cursor {
	...
	// Draw powerline (model + context metrics) between editor and help bar.
	if m.session != nil {
		if model := m.selectedLargeModel(); model != nil {
			supervisorTokens := m.session.PromptTokens + m.session.CompletionTokens
			holdActive := m.com.App.AgentCoordinator != nil && m.com.App.AgentCoordinator.IsHoldActive()
			var wtBranch string
			if m.com.App.AgentCoordinator != nil {
			...
			drawPowerline(scr, layout.powerline, m.com.Styles, model.Metadata.Name, model.Auth.HeaderDesignation(), supervisorTokens, model.Metadata.ContextWindow, m.session.TotalTokens, m.session.Cost, holdActive, wtBranch)
		}
	}
	...
	if os.Getenv("CRUCIBLE_UI_DEBUG") == "true" {
...
func (m *UI) View() tea.View {
	...
	v.WindowTitle = "crucible " + home.Short(m.com.Config().WorkingDir())
...
func (m *UI) generateLayout(w, h int) uiLayout {
	...
	// The help height
	helpHeight := 1
	// The powerline height (operational info bar, only in chat state)
	powerlineHeight := 0
	if m.state == uiChat {
		powerlineHeight = 1
	}
	...
	bottomHeight := helpHeight + powerlineHeight
	// Add app margins
	appRect, bottomRect := layout.SplitVertical(area, layout.Fixed(area.Dy()-bottomHeight))
	var powerlineRect, helpRect uv.Rectangle
	if powerlineHeight > 0 {
		powerlineRect, helpRect = layout.SplitVertical(bottomRect, layout.Fixed(powerlineHeight))
	} else {
		helpRect = bottomRect
	}
	appRect.Min.Y += 1
...
type uiLayout struct {
	...
	// powerline is the area for the operational info bar (model, context, cost).
	powerline uv.Rectangle
...
// editorMode represents the visual mode of the editor gutter.
type editorMode int
...
func resolveEditorMode(holdActive, yoloActive, shellPrefix bool, relayTarget *string) editorMode {
	...
	if holdActive {
		return editorModeHold
	}
	if yoloActive {
		return editorModeYolo
	}
...
// editorPromptFunc returns the editor gutter styling based on current hold/yolo
// state. Called by the textarea on every render — no explicit swaps needed.
func (m *UI) editorPromptFunc(info textarea.PromptInfo) string {
	...
	case editorModeHold:
		if info.LineNumber == 0 {
			if info.Focused {
				return t.EditorPromptHoldIconFocused.Render()
			}
			return t.EditorPromptHoldIconBlurred.Render()
		}
		if info.Focused {
			return t.EditorPromptHoldDotsFocused.Render()
		}
		return t.EditorPromptHoldDotsBlurred.Render()
	...
	case editorModeShell:
		if info.LineNumber == 0 {
			if info.Focused {
				return t.EditorPromptShellIconFocused.Render()
			}
			return t.EditorPromptShellIconBlurred.Render()
		}
		if info.Focused {
...
// isHoldActive returns true if hold mode is engaged. Safe to call at any
// point in the lifecycle (nil-guarded).
func (m *UI) isHoldActive() bool {
	return m.com.App != nil &&
		m.com.App.AgentCoordinator != nil &&
		m.com.App.AgentCoordinator.IsHoldActive()
}

// isYoloActive returns true if yolo mode (skip permission requests) is
// engaged. Safe to call at any point in the lifecycle (nil-guarded).
func (m *UI) isYoloActive() bool {
	return m.com.App != nil &&
		m.com.App.Permissions != nil &&
		m.com.App.Permissions.SkipRequests()
}

// toggleHold flips the hold flag and returns an info command. When setting
// hold, yolo is force-cleared — hold forces operator approval on every
// station dispatch until manually toggled off. Returns nil if no coordinator
// is available.
func (m *UI) toggleHold() tea.Cmd {
	coord := m.com.App.AgentCoordinator
	if coord == nil {
		return nil
	}
	if coord.IsHoldActive() {
		coord.ClearHold()
		return util.ReportInfo("Hold cleared")
	}
	coord.SetHold()
	if m.isYoloActive() {
		m.com.App.Permissions.SetSkipRequests(false)
		return util.ReportInfo("Hold set — yolo disabled, all station calls require approval until cleared")
	}
...
// turnMetricsPlaceholder returns the working placeholder enriched with
// per-request elapsed time and token count when available.
func (m *UI) turnMetricsPlaceholder() string {
	if m.session == nil || m.com.App == nil || m.com.App.AgentCoordinator == nil {
		return m.workingPlaceholder
	}
	tm := m.com.App.AgentCoordinator.TurnMetrics(m.session.ID)
	if tm == nil {
		return m.workingPlaceholder
	}
	elapsed := formatDuration(time.Since(tm.StartTime))
	// Use actual token count if available, otherwise estimate from streamed bytes (~4 bytes/token).
	tokenCount := tm.Usage.TotalTokens
	if tokenCount == 0 && tm.StreamedBytes > 0 {
		tokenCount = tm.StreamedBytes / 4
	}
	tokens := common.FormatTokenCount(tokenCount)
	return fmt.Sprintf("%s (%s · ↓ %s tokens)", m.workingPlaceholder, elapsed, tokens)
}
...
// ensureSession creates a Crucible session if one doesn't exist yet.
// Returns the session ID and any tea.Cmds needed (e.g. loadSession).
func (m *UI) ensureSession() (string, []tea.Cmd, error) {
	if m.hasSession() {
		return m.session.ID, nil, nil
	}
	newSession, err := m.com.App.Sessions.Create(context.Background(), agent.DefaultSessionName)
	if err != nil {
		return "", nil, err
	}
	if m.forceCompactMode {
	...
	if newSession.ID != "" {
		...
		m.pendingSessionID = newSession.ID
		cmds = append(cmds, m.loadSession(newSession.ID))
	}
	m.setState(uiChat, m.focus)
	return m.session.ID, cmds, nil
}

// executeShellCommand runs a shell command directly via the coordinator.
func (m *UI) executeShellCommand(cmd string) tea.Cmd {
	if m.com.App.AgentCoordinator == nil {
		return util.ReportError(fmt.Errorf("coder agent is not initialized"))
	}
	sessionID, cmds, err := m.ensureSession()
	if err != nil {
		return util.ReportError(err)
	}
	m.randomizePlaceholders()
	...
	cmds = append(cmds, func() tea.Msg {
		err := m.com.App.AgentCoordinator.ExecuteUserShell(context.Background(), sessionID, cmd)
...
func (m *UI) cancelAgent() tea.Cmd {
	...
	if m.isCanceling {
		...
		// Immediately cancel all spinning tool items (belt-and-suspenders:
		// the FinishReasonCanceled message may not have arrived yet).
		m.cancelSpinningTools()
...
// cancelSpinningTools marks all in-flight tool items as canceled so their
// spinners stop immediately. Called from the escape handler to cover the race
// where the agent's FinishReasonCanceled message hasn't arrived yet.
func (m *UI) cancelSpinningTools() {
	for i := 0; i < m.chat.Len(); i++ {
		item := m.chat.ItemAt(i)
		if toolItem, ok := item.(chat.ToolMessageItem); ok {
			if toolItem.Status() == chat.ToolStatusRunning {
				toolItem.SetStatus(chat.ToolStatusCanceled)
			}
		}
	}
...
func (m *UI) openDialog(id string) tea.Cmd {
	...
	case dialog.ThemeDialogID:
		if cmd := m.openThemeDialog(); cmd != nil {
			cmds = append(cmds, cmd)
		}
	case dialog.SpinnerDialogID:
		if cmd := m.openSpinnerDialog(); cmd != nil {
			cmds = append(cmds, cmd)
		}
	case dialog.QuitID:
		if cmd := m.openQuitDialog(); cmd != nil {
...
func (m *UI) openThemeDialog() tea.Cmd {
	if m.dialog.ContainsDialog(dialog.ThemeDialogID) {
		m.dialog.BringToFront(dialog.ThemeDialogID)
		return nil
	}
...
// applyTheme persists the selected theme and hot-reloads all UI components.
func (m *UI) applyTheme(themeID string) tea.Cmd {
	// 1. Persist to config
	if err := m.com.Config().SetTheme(themeID); err != nil {
		return util.ReportError(err)
	}

	// 2. Rebuild styles and replace in-place (all components share *com.Styles)
	newStyles := styles.NewStyles(styles.ThemeID(themeID), m.isTransparent)
	newStyles.SpinnerPreset = m.com.Config().SpinnerPreset() // preserve spinner choice
	*m.com.Styles = newStyles

	// 3. applyStyles: restyle long-lived components that cache style values at init
	...
	m.todoSpinner = spinner.New(
		...
		spinner.WithStyle(m.com.Styles.Pills.TodoSpinner),
	)
	m.status.SetStyles(m.com.Styles)

	var cmds []tea.Cmd

	// 4. Restart the new spinner if one was already spinning
	if m.todoIsSpinning {
		cmds = append(cmds, m.todoSpinner.Tick)
	}

	// 5. Rebuild chat model from current session messages to recolor items,
	//    preserving the user's scroll position and selected item.
	if m.hasSession() && m.session != nil {
		wasAtBottom := m.chat.AtBottom()
		prevSelected := m.chat.SelectedIndex()

		msgs, err := m.com.App.Messages.List(context.Background(), m.session.ID)
		if err == nil {
			m.setSessionMessages(msgs)

			// Restore scroll position: only scroll to bottom if user was
			// already following; otherwise put them back where they were.
			if !wasAtBottom {
				m.chat.SetSelected(prevSelected)
				m.chat.ScrollToSelected()
			}
		}
	}

	// 6. Invalidate all cached renders (header: both wide and compact)
	m.header.InvalidateCache()
	m.sidebarLogo = ""
	m.pillsView = ""
	...
	cmds = append(cmds, util.ReportInfo(fmt.Sprintf("Theme changed to %s", dialog.ThemeDisplayName(themeID))))
	return tea.Batch(cmds...)
}

// openSpinnerDialog opens the spinner picker dialog.
func (m *UI) openSpinnerDialog() tea.Cmd {
	if m.dialog.ContainsDialog(dialog.SpinnerDialogID) {
		m.dialog.BringToFront(dialog.SpinnerDialogID)
		return nil
	}

	d, err := dialog.NewSpinnerPicker(m.com)
	if err != nil {
		return util.ReportError(err)
	}
...
// applySpinner persists the selected spinner preset and rebuilds chat messages.
func (m *UI) applySpinner(preset string) tea.Cmd {
	// 1. Persist to config
	if err := m.com.Config().SetSpinner(preset); err != nil {
		return util.ReportError(err)
	}

	// 2. Update styles (carries preset to call sites on message rebuild)
	m.com.Styles.SpinnerPreset = preset

	var cmds []tea.Cmd

	// 3. Rebuild chat messages from session (recreates spinner backends).
	//    MUST capture the returned tea.Cmd — it contains StartAnimation()
	//    commands for all active spinners. Without this, spinners freeze.
	if m.hasSession() && m.session != nil {
		wasAtBottom := m.chat.AtBottom()
		prevSelected := m.chat.SelectedIndex()
		msgs, err := m.com.App.Messages.List(context.Background(), m.session.ID)
	...
	cmds = append(cmds, util.ReportInfo(fmt.Sprintf("Spinner changed to %s", dialog.SpinnerDisplayName(preset))))
	return tea.Batch(cmds...)
}
...
// openArtifactsDialog opens the artifact viewer dialog for the current session.
func (m *UI) openArtifactsDialog() tea.Cmd {
	if m.dialog.ContainsDialog(dialog.ArtifactsID) {
		m.dialog.BringToFront(dialog.ArtifactsID)
		return nil
	}
	...
	m.dialog.OpenDialog(dialog.NewArtifacts(m.com, m.com.App.ArtifactService(), sessionID))
	return nil
}

// openEquipmentDialog opens the equipment inspector dialog.
func (m *UI) openEquipmentDialog() tea.Cmd {
	if m.dialog.ContainsDialog(dialog.EquipmentID) {
		m.dialog.BringToFront(dialog.EquipmentID)
		return nil
	}
	m.dialog.OpenDialog(dialog.NewEquipment(m.com, m.mcpStates))
	return nil
}
...
// openAskUserDialog opens the ask_user dialog for a structured question request.
func (m *UI) openAskUserDialog(req askuser.Request) tea.Cmd {
	// Mark the chat card as awaiting input.
	if req.ToolCallID != "" {
		if item := m.chat.MessageItem(req.ToolCallID); item != nil {
			if toolItem, ok := item.(chat.ToolMessageItem); ok {
				toolItem.SetStatus(chat.ToolStatusAwaitingInput)
			}
		}
	}
	m.dialog.CloseDialog(dialog.AskUserID)
	d := dialog.NewAskUser(m.com, req)
	m.dialog.OpenDialog(d)
	return nil
}
...
func (m *UI) newSession() tea.Cmd {
	...
	m.pillsView = ""
	m.historyReset()
	// TODO: ResetCache was removed with agent/tools — re-add if needed
...
func (m *UI) drawSessionDetails(scr uv.Screen, area uv.Rectangle) {
	...
	filesSection := m.filesInfo(m.com.Config().WorkingDir(), getFilesWithChanges(m.sessionFiles), sectionWidth, maxItemsPerSection, false)
...
// renderLogo renders the Crucible logo with the given styles and dimensions.
func renderLogo(t *styles.Styles, compact bool, width int) string {
	return logo.Render(t, version.Version, compact, logo.Opts{
		FieldColor:   t.LogoFieldColor,
		StrokeColor:  t.BgBase,
