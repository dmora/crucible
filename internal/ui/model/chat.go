...
func (m *Chat) SetSize(width, height int) {
	...
	// Follow mode is set during session reload and streaming; without this
	// check, a layout change after ScrollToBottom() can leave the viewport
	// stranded above the last message.
...
func (m *Chat) Animate(msg anim.StepMsg) tea.Cmd {
	...
	if !ok {
		delete(m.pausedAnimations, msg.ID) // cleanup zombie entry
		return nil
	}
