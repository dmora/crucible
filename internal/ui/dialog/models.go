...
type Models struct {
	...
	modelType ModelType
	providers []config.ProviderMetadata
...
func (m *Models) isSelectedConfigured() bool {
	...
	_, isConfigured := m.com.Config().Providers.Get(modelItem.prov.ID)
	return isConfigured
}
...
func (m *Models) setProviderItems() error {
	...
	containsProviderFunc := func(id string) func(p config.ProviderMetadata) bool {
		return func(p config.ProviderMetadata) bool {
			return p.ID == id
		}
	}
	...
	for id, p := range cfg.Providers.Seq2() {
		...
		if !slices.ContainsFunc(knownProviders, containsProviderFunc(id)) ||
			...
			provider := p.ToProviderMetadata(id)
			...
			for _, model := range p.Models {
				...
				if model.ID == currentModel.Model && provider.ID == currentModel.Provider {
	...
	for _, provider := range m.providers {
		...
		providerConfig, providerConfigured := cfg.Providers.Get(provider.ID)
		...
		name := cmp.Or(displayProvider.Name, provider.ID)
