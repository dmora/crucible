...
func (p *Permissions) renderHeader(contentWidth int) string {
	...
	// Gate requests use a distinct title so the operator knows this is a
	// station gate, not a regular tool permission.
	dialogTitle := "Permission Required"
	if station, ok := strings.CutPrefix(p.permission.ToolName, "gate:"); ok {
		dialogTitle = "Station Gate: " + strings.ToUpper(station)
	}

	title := common.DialogTitle(t, dialogTitle, contentWidth-t.Dialog.Title.GetHorizontalFrameSize(), t.Primary, t.Secondary)
...
func (p *Permissions) renderDefaultContent(width int) string {
	...
	if p.permission.Params != nil {
		...
		if str, ok := p.permission.Params.(string); ok {
			// String params: parse → pretty-print as before.
			paramStr = str
			var parsed any
			...
			// Non-string params (map, struct): marshal directly — no round-trip.
			if b, err := json.MarshalIndent(p.permission.Params, "", "  "); err == nil {
				paramStr = string(b)
			} else {
