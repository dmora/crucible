...
func (c *Commands) defaultCommands() []*CommandItem {
	...
	if c.hasSession {
		cfg := c.com.Config()
		agentCfg := cfg.Agents[config.AgentCrucible]
		model := cfg.GetModelByType(agentCfg.Model)
		if model != nil && model.SupportsAttachments {
	...
	commands = append(commands,
		NewCommandItem(c.com.Styles, "switch_theme", "Switch Theme", "ctrl+y", ActionOpenDialog{
			DialogID: ThemeDialogID,
		}),
		NewCommandItem(c.com.Styles, "switch_spinner", "Switch Spinner", "", ActionOpenDialog{
