...
// clients within the Crucible application.
package mcp
...
var (
	...
	// Registry is the global MCP toolset registry for ADK integration.
	// Initialized during Initialize() and used by agent.go to wire MCP
	// tools into the ADK agent's Toolsets.
...
func Initialize(ctx context.Context, permissions permission.Service, cfg *config.Config) {
	...
	Registry = NewRegistry(cfg, permissions)
	...
	for name, m := range cfg.MCP {
		...
		if strings.Contains(name, "_") {
			slog.Error("MCP server name contains underscores; use hyphens instead to avoid tool name collisions",
				"name", name, "suggestion", strings.ReplaceAll(name, "_", "-"))
			updateState(name, StateError, fmt.Errorf("server name %q contains underscores", name), nil, Counts{})
			continue
		}
...
func getOrRenewClient(ctx context.Context, cfg *config.Config, name string) (*ClientSession, error) {
	...
	// Refresh tool/prompt/resource caches after reconnect.
	if err := refreshServerState(ctx, cfg, name, sess); err != nil {
		return nil, err
	}
...
// refreshServerState re-lists tools, prompts, and resources after a
// reconnection and updates caches and counts. If tool listing fails
// the server is left in StateError since tools are required for LLM use.
func refreshServerState(ctx context.Context, cfg *config.Config, name string, sess *ClientSession) error {
	tools, err := getTools(ctx, sess)
	if err != nil {
		refreshErr := fmt.Errorf("reconnect: list tools: %w", err)
		slog.Warn("Reconnect: failed to list tools", "name", name, "error", err)
		updateState(name, StateError, refreshErr, nil, Counts{})
		return refreshErr
	}
	toolCount := updateTools(cfg, name, tools)

	prompts, err := getPrompts(ctx, sess)
	if err != nil {
		slog.Warn("Reconnect: failed to list prompts", "name", name, "error", err)
	}
	updatePrompts(name, prompts)

	resources, err := getResources(ctx, sess)
	if err != nil {
		slog.Warn("Reconnect: failed to list resources", "name", name, "error", err)
	}
	resourceCount := updateResources(name, resources)
...
func updateState(name string, state State, err error, client *ClientSession, counts Counts) {
	...
	case StateError:
		sessions.Del(name)
		// Clear stale tool/prompt/resource caches for this server.
		allTools.Del(name)
		allPrompts.Del(name)
