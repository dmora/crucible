...
import (
	...
	"github.com/dmora/crucible/internal/skills"
)
...
type PromptDat struct {
	AgentID       string // Current agent's key in Config.Agents (used to skip self in template)
	Provider      string
	Model         string
...
func (p *Prompt) Build(ctx context.Context, agentID, provider, model string, cfg config.Config) (string, error) {
	t, err := template.New(p.name).Parse(p.template)
	if err != nil {
		return "", fmt.Errorf("parsing template: %w", err)
	}
	var sb strings.Builder
	d, err := p.promptData(ctx, agentID, provider, model, cfg)
	if err != nil {
		return "", err
	}
...
func (p *Prompt) promptData(ctx context.Context, agentID, provider, model string, cfg config.Config) (PromptDat, error) {
	workingDir := cmp.Or(p.workingDir, cfg.WorkingDir())
	platform := cmp.Or(p.platform, runtime.GOOS)

	files := map[string][]ContextFile{}

	// Phase 1: Resolve tiered default context paths (first matching tier wins).
	for _, tier := range config.DefaultContextTiers() {
		tierHasFiles := false
		for _, pth := range tier.Paths {
			expanded := expandPath(pth, cfg)
			pathKey := strings.ToLower(expanded)
			if _, ok := files[pathKey]; ok {
				continue
			}
			content := processContextPath(expanded, workingDir)
			if len(content) > 0 {
		...
		if tierHasFiles {
			break // First tier with any existing file wins; skip remaining tiers
		}
	}

	// Phase 2: Always load user-configured context paths (explicit, not tiered).
