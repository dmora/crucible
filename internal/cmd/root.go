...
import (
	...
	ui "github.com/dmora/crucible/internal/ui/model"
...
func init() {
	...
	rootCmd.PersistentFlags().StringP("data-dir", "D", "", "Custom crucible data directory")
	rootCmd.PersistentFlags().BoolP("debug", "d", false, "Debug")
	rootCmd.Flags().BoolP("help", "h", false, "Help")
	rootCmd.Flags().BoolP("yolo", "y", false, "Automatically accept all permissions (dangerous mode)")
	rootCmd.PersistentFlags().Bool("worktree", false, "Enable git worktree isolation per session")
...
crucible -d -c /path/to/project

# Run with custom data directory
crucible -D /path/to/custom/.crucible

# Print version
crucible -v

# Run a single non-interactive prompt
crucible run "Explain the use of context in Go"
  ...
  `,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if _, err := program.Run(); err != nil {
			...
			return errors.New("Crucible crashed. If metrics are enabled, we were notified about it. If you'd like to report it, please copy the stacktrace above and open an issue at https://github.com/dmora/crucible/issues/new?template=bug.yml") //nolint:staticcheck
		}
		return nil
	},
}
...
func setupApp(cmd *cobra.Command) (*app.App, error) {
	...
	if worktree {
		cfg.Options.Worktree = true
	}

	if err := createDotCrucibleDir(cfg.Options.DataDirectory); err != nil {
		return nil, err
	}
...
func shouldEnableMetrics(cfg *config.Config) bool {
	if v, _ := strconv.ParseBool(os.Getenv("CRUCIBLE_DISABLE_METRICS")); v {
		return false
	}
...
func createDotCrucibleDir(dir string) error {
