...
func TestAppendSlice_NewKey(t *testing.T) {
	t.Parallel()

	m := NewMap[string, []int]()
	AppendSlice(m, "key1", 42)

	val, ok := m.Get("key1")
	require.True(t, ok)
	require.Equal(t, []int{42}, val)
}

func TestAppendSlice_ExistingKey(t *testing.T) {
	t.Parallel()

	m := NewMap[string, []int]()
	AppendSlice(m, "key1", 1)
	AppendSlice(m, "key1", 2)
	AppendSlice(m, "key1", 3)
...
func TestAppendSlice_Concurrent(t *testing.T) {
	t.Parallel()
	...
	val, ok := m.Get("key")
	require.True(t, ok)
	require.Len(t, val, numGoroutines, "no elements should be lost under concurrent appends")
}

func TestPrependSlice_NewKey(t *testing.T) {
	t.Parallel()

	m := NewMap[string, []int]()
	PrependSlice(m, "key1", []int{1, 2})
...
func TestPrependSlice_ExistingKey(t *testing.T) {
	t.Parallel()
	...
	val, ok := m.Get("key1")
	require.True(t, ok)
	require.Equal(t, []int{1, 2, 3, 4}, val)
}

func TestPrependSlice_Empty(t *testing.T) {
	t.Parallel()

	m := NewMap[string, []int]()
	m.Set("key1", []int{1})
	PrependSlice(m, "key1", nil)

	val, ok := m.Get("key1")
	require.True(t, ok)
	require.Equal(t, []int{1}, val, "empty prepend should not alter existing slice")
}

func TestPrependSlice_Concurrent(t *testing.T) {
	t.Parallel()
	...
	val, ok := m.Get("key")
	require.True(t, ok)
	require.Len(t, val, numGoroutines, "no elements should be lost under concurrent prepends")
}
...
func TestMap_Swap_ExistingKey(t *testing.T) {
	t.Parallel()
...
func TestMap_Swap_Concurrent(t *testing.T) {
	t.Parallel()
...
func TestMap_DeleteFunc_Match(t *testing.T) {
	t.Parallel()

	m := NewMap[string, int]()
	m.Set("a", 42)

	deleted := m.DeleteFunc("a", func(v int) bool { return v == 42 })
	assert.True(t, deleted)
	assert.Equal(t, 0, m.Len())
}

func TestMap_DeleteFunc_NoMatch(t *testing.T) {
	t.Parallel()

	m := NewMap[string, int]()
	m.Set("a", 42)

	deleted := m.DeleteFunc("a", func(v int) bool { return v == 99 })
...
func TestMap_DeleteFunc_MissingKey(t *testing.T) {
	t.Parallel()

	m := NewMap[string, int]()

	deleted := m.DeleteFunc("a", func(v int) bool { return true })
	assert.False(t, deleted)
}
...
func TestMap_DeleteFunc_Concurrent(t *testing.T) {
	...
	for i := range numItems {
		go func(v int) {
			...
			if m.DeleteFunc(key, func(val int) bool { return val == v }) {
				wins.Add(1)
			}
		}(i)
	}
	...
	assert.Equal(t, int32(numItems), wins.Load())
	assert.Equal(t, 0, m.Len())
}
