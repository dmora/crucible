...
// Swap atomically sets key to value and returns the previous value, if any.
func (m *Map[K, V]) Swap(key K, value V) (old V, loaded bool) {
	m.mu.Lock()
	defer m.mu.Unlock()
	old, loaded = m.inner[key]
	m.inner[key] = value
	return
}

// DeleteFunc atomically deletes key if pred returns true for the current value.
// Returns true if the entry was deleted.
func (m *Map[K, V]) DeleteFunc(key K, pred func(V) bool) bool {
	m.mu.Lock()
	defer m.mu.Unlock()
	if v, ok := m.inner[key]; ok && pred(v) {
		delete(m.inner, key)
		return true
	}
	return false
...
// Returns true if the value was set (key was absent), false if it already existed.
...
// AppendSlice atomically appends elem to the slice stored at key.
// This is a package-level function because Go generics cannot constrain
// a method's value type to be a slice on an existing Map[K, V].
func AppendSlice[K comparable, E any](m *Map[K, []E], key K, elem E) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.inner[key] = append(m.inner[key], elem)
}

// PrependSlice atomically prepends elems before any existing elements at key.
// Used to restore failed items to the front of a queue without losing
// concurrently-appended entries.
func PrependSlice[K comparable, E any](m *Map[K, []E], key K, elems []E) {
	if len(elems) == 0 {
		return
	}
	m.mu.Lock()
	defer m.mu.Unlock()
	m.inner[key] = append(elems, m.inner[key]...)
}
